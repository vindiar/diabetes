...
Mencakup: ML Lifecycle, Preprocessing, Evaluasi, Cara Baca Visualisasi, Algoritma
"""
...
with st.expander("📋 Daftar Isi — klik untuk expand", expanded=False):
    ...
    3. [ML Development Lifecycle](#3-ml-development-lifecycle)
       - Fase 1 — Data & EDA
       - Fase 2 — Load Model
       - Fase 3 — Evaluasi
       - Fase 4 — klasifikasi / Serving
...
st.markdown("""
Aplikasi ini adalah sistem **klasifikasi risiko penyakit diabetes** berbasis Machine Learning yang dibangun menggunakan **Streamlit**, mengikuti alur standar **ML Development Lifecycle**.
...
| `blood_glucose_level` | mg/dL | Gula darah pemerikasaan |
""")
...
# 3. ML LIFECYCLE
# ─────────────────────────────────────────────────────────────────────────────
st.markdown("### 3️⃣ ML Development Lifecycle")

tab1, tab2, tab3, tab4 = st.tabs(["📊 Fase 1 — Data & EDA", "💾 Fase 2 — Load Pre-trained", "📈 Fase 3 — Evaluasi", "🔍 Fase 4 — klasifikasi"])

with tab1:
    st.markdown("""
#### Fase 1 — Data Loading & Exploratory Data Analysis (EDA)
**File:** `utils/data.py` | **Halaman:** Dashboard & EDA

**Tujuan:** Mempersiapkan data uji coba yang bersih untuk dievaluasi oleh dashboard secara interaktif.

**Langkah:**
1. **Load Dataset** — Membaca `data/balanced_dataset.csv` dengan Pandas, di-cache via `@st.cache_data`
2. **Filter** — Fitur jenis kelamin dibersihkan sehingga murni mencakup 'Male' dan 'Female'.
3. **Outlier Removal** — Menerapkan teknik rentang **IQR (Q1-Q3)** guna membuang pencilan numerik secara ketat.

> Hal ini memastikan bahwa data berukuran 12.680 baris yang dikonstruksi secara programatik *sama persis kualitasnya* dengan data ketika dilatih di Jupiter Notebook peneliti.
""")

with tab2:
    st.markdown("""
#### Fase 2 — Load Pre-Trained Model
**Lokasi Artefak:** `models/`

**Artefak yang Digunakan:**
Model dieksekusi secara instan dengan menggunakan metode desialisasi pickle(`.pkl`):
| File .pkl | Nilai | Keterangan |
|-----------|-------|-----------|
| `model_diabetes.pkl` | **1 MB** | Bobot algoritma Logistic Regression Model yang sudah paten. |
| `scaler_diabetes.pkl` | **< 1 KB** | Referensi mean dan variance Standardization |
| `features_diabetes.pkl` | **< 1 KB** | Struktur ekspektasi One-Hot array |

**Sistem Kerja:**
Aplikasi tidak lagi perlu menjalankan fungsi `.fit()`. Menggunakan model persisten (*pre-trained*) memastikan tingkat kestabilan eksekusi maksimal.
""")
...
| Metrik (Test Set 20%) | Evaluasi | Keterangan Khusus |
|--------|-------|-----------|
| **AUC-ROC** | **0.9338** | Eksekusi Sangat Baik (Excellent) |
| **Akurasi** | **0.8458** | Klasifikasi Tepat Keseluruhan |
| 5-Fold Cross Validation | 0.8562 | Rata-rata akurasi CV |

> Di konteks medis, **Recall (Sensitivitas)** juga merupakan indikator krusial dalam mempertimbangkan resiko luput (*False Negative*). Berbanding terbalik dengan itu, tingkat kestabilan kurva ROC mencerminkan bahwa penarikan rentang batas peluang jauh di atas kemampuan estimasi rambang (*random guess*).
""")

with tab4:
    st.markdown("""
#### Fase 4 — Single-Patient Prediction (Serving)
**File:** `utils/model.py` | **Halaman:** klasifikasi

**Alur Inferensi (untuk 1 pasien baru):**
1. **Input Maping** — Pengguna mengisikan Berat serta Tinggi. Sistem otomasi mengekstraksi BMI.
2. **One-Hot Arraying** — Fitur *Gender* dan *Histori Merokok* diadaptasikan ke bentuk One-Hot Array yang tepat menggunakan formasi statik `features_diabetes.pkl` (menyiapkan array 0,1 untuk `smoking_history_current`, dan lainnya demi memuaskan ekspektasi 12 elemen model).
...
- ✅ **Probabilistik** — Murni merilis skala 0 sampai 100%, sangat relevan untuk asesmen resiko penangan pasien klinis di dunia modern.
""")
...
- *HbA1c* (Gula darah 3 bln) dan *Blood Glucose* tampil sebagai salah satu prediktor berkorelasi langsung linear tertinggi dengan variabel terikta (target) di struktur kami.
""")
...
Aplikasi menggunakan metode cerdas di mana bagian kalkulasi evaluasi grafikal tidak dilakukan *On the Fly*. Hanya fungsi ramalan resiko individu (*page_predict*) yang meregulasi probabilitas instan.
""", icon="🏗️")
